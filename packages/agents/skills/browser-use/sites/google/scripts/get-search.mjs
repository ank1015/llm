...
async function runSearch(chrome, options) {
  ...
  if (workingTabId !== null) {
    await callChrome(chrome, 'tabs.get', workingTabId);
    log(`using existing tab ${workingTabId}`);
    await callChrome(chrome, 'tabs.update', workingTabId, {
...
async function createWorkingTab(chrome, url) {
  try {
    const created = await callChrome(chrome, 'tabs.create', {
      url,
      active: true,
    });
  ...
  const createdWindow = await callChrome(chrome, 'windows.create', {
    url,
    focused: true,
    type: 'normal',
  });
...
async function extractSearchPage(chrome, tabId, timeoutMs) {
...
async function goToNextResultsPage(chrome, tabId) {
  const response = await callChrome(chrome, 'debugger.evaluate', {
    tabId,
...
function buildAdvancedSearchSubmitCode(options, numPerPage) {
...
function determineSearchMode(options) {
...
function buildSearchSummary(options) {
  const parts = [];
...
async function waitForTabLoad(chrome, tabId, timeoutMs) {
  const deadline = Date.now() + timeoutMs;
  ...
  while (Date.now() < deadline) {
    ...
    if (tab.status === 'complete') {
      await sleep(250);
      const settled = await callChrome(chrome, 'tabs.get', tabId);
...
async function callChrome(chrome, method, ...args) {
  return await chrome.call(method, ...args);
}
...
function buildHelpText() {
  return [
    'Google getSearch',
    '',
    'Usage: node .max/skills/browser-use/sites/google/scripts/get-search.mjs [options]',
    '',
    'Provide --query or at least one of --all-words, --exact-phrase, or --any-words.',
    '',
    'Core options:',
    ...
    '  --include-sponsored <true|false>  default: true',
    '  --exclude-sponsored',
    '  --json-output <path>',
