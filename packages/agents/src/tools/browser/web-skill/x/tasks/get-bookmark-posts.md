# Task: getBookmarkPosts

File: `/Users/notacoder/Desktop/agents/llm/packages/agents/src/tools/browser/web-skill/x/tasks/get-bookmark-posts.md`  
Works on URLs:

- `https://x.com/i/bookmarks`

Use this snippet to collect posts from X bookmarks with virtualized timeline scrolling.
...
```js
const EXTRACT_BOOKMARKS = String.raw`(() => {
  ...
  for (const article of articles) {
    ...
    const statusAnchor =
      timeLink && timeLink.closest('a[href*="/status/"]')
        ? timeLink.closest('a[href*="/status/"]')
...
async function getBookmarkPosts(input) {
  ...
  await window.open('https://x.com/i/bookmarks', {
    newTab: false,
    active: true,
    timeoutMs: 40_000,
  });
  ...
  for (let cycle = 0; cycle < maxScrolls && results.length < want; cycle++) {
    const page = await window.evaluate(EXTRACT_BOOKMARKS, { timeoutMs: 30_000 });
