# Task: getHomeFeedPosts

File: `/Users/notacoder/Desktop/agents/llm/packages/agents/src/tools/browser/web-skill/x/tasks/get-home-feed-posts.md`  
Works on URLs:

- `https://x.com/home`
- `https://x.com/home?f=live`

Use this snippet to collect posts from X home feed while handling virtualized DOM behavior with natural scrolling.
...
```js
const EXTRACT_HOME_FEED = String.raw`(() => {
  ...
  const detectLoginRequired = () => {
    if (window.location.pathname.indexOf('/i/flow/login') >= 0) {
      return true;
    }
  ...
  const feedKind = new URL(window.location.href).searchParams.get('f') === 'live' ? 'following' : 'for_you';
  ...
  for (const article of articles) {
    ...
    const authorHandle =
      userNameBlock instanceof HTMLElement
        ...
        : status && status.authorHandleFromUrl
          ? status.authorHandleFromUrl
          : '';
    ...
    const raw = clean(article.textContent || '').slice(0, 2000);
    ...
    const quote = article.querySelector('[role="blockquote"]') !== null;

    const replies = getMetric(article, 'reply');
    const reposts = getMetric(article, 'retweet');
    const likes = getMetric(article, 'like');
    const bookmarks = getMetric(article, 'bookmark');
    const views = getMetric(article, 'viewCount');

    if (!text && !status && !authorHandle) continue;
...
function buildHomeUrl(mode) {
  return mode === 'following' ? 'https://x.com/home?f=live' : 'https://x.com/home';
}
...
async function getHomeFeedPosts(input) {
  const modeRaw = input && typeof input.mode === 'string' ? input.mode : 'for_you';
  const mode = modeRaw === 'following' ? 'following' : 'for_you';
  ...
  await window.open(buildHomeUrl(mode), {
    newTab: false,
    active: true,
    timeoutMs: 40_000,
  });
  ...
  if (readiness && readiness.loginRequired === true) {
    loginRequired = true;
  }

  for (let cycle = 0; cycle < maxScrolls && results.length < want; cycle++) {
    const page = await window.evaluate(EXTRACT_HOME_FEED, { timeoutMs: 30_000 });

    const pageInfo = page && page.page ? page.page : {};
    const items = page && Array.isArray(page.items) ? page.items : [];
    const visibleArticleCount =
      page && typeof page.visibleArticleCount === 'number' ? page.visibleArticleCount : 0;

    loginRequired = pageInfo.loginRequired === true;

    let added = 0;
    for (const item of items) {
      if (!includePromoted && item.promoted === true) continue;
    ...
    cycles.push({
      ...
      moved,
      stagnantCycles,
      atBottom: scrollMeta && scrollMeta.atBottom === true,
    });
