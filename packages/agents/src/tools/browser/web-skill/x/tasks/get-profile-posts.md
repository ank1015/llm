# Task: getProfilePosts

File: `/Users/notacoder/Desktop/agents/llm/packages/agents/src/tools/browser/web-skill/x/tasks/get-profile-posts.md`  
Works on URLs:
...
Use this snippet to collect posts from an X profile timeline tab (posts, replies, media).
...
```js
const EXTRACT_PROFILE_TIMELINE = String.raw`(() => {
...
const TIMELINE_STATE = String.raw`(() => {
  ...
  return {
    ...
    url: window.location.href,
    title: document.title,
    hasProfileHeader: document.querySelector('[data-testid="UserName"]') !== null,
  };
})()`;
...
function buildProfileTimelineUrl(handleOrUrl, tab) {
  const handle = normalizeHandle(handleOrUrl);
  if (!handle) throw new Error('handleOrUrl is required');

  if (tab === 'replies') return 'https://x.com/' + handle + '/with_replies';
  if (tab === 'media') return 'https://x.com/' + handle + '/media';
...
async function getProfilePosts(input) {
  const tabRaw = input && typeof input.tab === 'string' ? input.tab : 'posts';
  const tab = tabRaw === 'replies' || tabRaw === 'media' ? tabRaw : 'posts';
  ...
  const url = buildProfileTimelineUrl(input && input.handleOrUrl, tab);
  ...
  for (let cycle = 0; cycle < maxScrolls && results.length < want; cycle++) {
    const page = await window.evaluate(EXTRACT_PROFILE_TIMELINE, { timeoutMs: 30_000 });
