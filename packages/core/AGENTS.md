...
Stateless runtime package for chat/image model catalogs, provider dispatch, normalized message/event contracts, and the shared agent engine.
...
- `src/types/` - normalized types for messages, content, image results, tools, agent contracts, and provider option maps
