...
Stateless runtime package for model catalogs, provider dispatch, normalized message/event contracts, and the shared agent engine.
...
- `src/types/` - normalized types for messages, content, tools, agent contracts, and provider option maps
...
- When adding providers, wire model metadata, provider registration, normalized streaming behavior, and tests together.
