...
import { buildParams, createClient, getMockKimiMessage, mapStopReason } from "./utils.js";
	...
	(async () => {
		...
		try {
			...
			for await (const chunk of kimiStream) {
				...
				const usageData = chunk.usage || (choice as typeof choice & { usage?: typeof chunk.usage }).usage;
				if (usageData) {
