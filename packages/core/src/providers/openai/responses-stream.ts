...
interface StreamOpenAIResponsesConfig<
  TApi extends OpenAIResponsesApi,
  TOptions extends OpenAIResponsesStreamOptions,
> {
  model: Model<TApi>;
  context: Context;
  options: TOptions;
  id: string;
  providerName: string;
  createClient(model: Model<TApi>, options: TOptions): OpenAIResponsesClient;
  buildParams(
    model: Model<TApi>,
    context: Context,
    options: TOptions
  ): ResponseCreateParamsNonStreaming;
  getMockNativeMessage(modelId: string, requestId: string): Response;
}
...
}: StreamOpenAIResponsesConfig<TApi, TOptions>): AssistantMessageEventStream<TApi> {
  const stream = new AssistantMessageEventStream<TApi>();

  // Shared OpenAI Responses event normalization mirrors the original provider stream loop.
  // eslint-disable-next-line sonarjs/cognitive-complexity
  (async (): Promise<void> => {
    const startTimestamp = Date.now();
    let finalResponse: Response = getMockNativeMessage(model.id, id);
    ...
    const syncResponseUsage = (response: Response): void => {
    ...
    try {
      ...
      const openaiStream = await client.responses.create(paramsStreaming, requestOptions);
      ...
      for await (const event of openaiStream) {
        if (event.type === 'response.output_item.added') {
          ...
          throw (
            finalResponse.error || {
              message: `${providerName} response failed without error details`,
            }
          );
        }
      }
