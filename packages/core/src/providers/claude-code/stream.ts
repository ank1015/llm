...
export const streamClaudeCode: StreamFunction<'claude-code'> = (
  model: Model<'claude-code'>,
  context: Context,
  options: ClaudeCodeProviderOptions,
  id: string
) => {
  const stream = new AssistantMessageEventStream<'claude-code'>();

  (async () => {
    const startTimestamp = Date.now();
    const finalResponse: AnthropicMessage = getMockClaudeCodeMessage(model.id, id);

    const output: BaseAssistantMessage<'claude-code'> = {
      role: 'assistant',
      api: model.api,
      model: model,
      id,
