...
export const streamCodex: StreamFunction<'codex'> = (
  model: Model<'codex'>,
  context: Context,
  options: CodexProviderOptions,
  id: string
) => {
  const stream = new AssistantMessageEventStream<'codex'>();

  (async () => {
    const startTimestamp = Date.now();
    let finalResponse: Response = getMockCodexMessage(model.id, id);

    const output: BaseAssistantMessage<'codex'> = {
      role: 'assistant',
      api: model.api,
      model: model,
      id,
    ...
    try {
      ...
      const codexStream = await client.responses.create(paramsStreaming, {
