...
export function getMockOpenRouterMessage(modelId: string, requestId: string): ChatCompletion {
...
export function buildParams(
  ...
  const messages = buildOpenRouterMessages(model, context);

  const { apiKey, signal, ...openrouterOptions } = options;
  ...
  if (context.tools && context.tools.length > 0 && model.tools.includes('function_calling')) {
    ...
    if (openrouterOptions.tools) {
      for (const optionTool of openrouterOptions.tools) {
...
export function buildOpenRouterMessages(
  ...
  for (const message of context.messages) {
    ...
    if (message.role === 'assistant') {
      if (message.model.api === 'openrouter') {
        // Native OpenRouter message - reconstruct from original
        const baseMessage = message as BaseAssistantMessage<'openrouter'>;
