			...
			if (candidate.content?.parts) {
				for (const part of candidate.content.parts) {
					...
					if (part.functionCall) {
						const toolCallId = part.functionCall.id || `${part.functionCall.name}_${Date.now()}_${Math.random()}`;
...
export function getResponseUsage(response: GenerateContentResponse, model: Model<"google">): Usage {
	...
	const input = (response.usageMetadata?.promptTokenCount || 0) - (response.usageMetadata?.cachedContentTokenCount || 0);
...
export function transformSchemaForGoogle(schema: JSONSchemaValue): JSONSchemaValue {
	...
	if ("anyOf" in schema && Array.isArray(schema.anyOf) && schema.anyOf.length > 0) {
		const allConst = schema.anyOf.every(
			(item: JSONSchemaValue) => item && typeof item === "object" && !Array.isArray(item) && "const" in item,
		);
		if (allConst) {
