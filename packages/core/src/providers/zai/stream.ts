...
export const streamZai: StreamFunction<"zai"> = (
	...
	const stream = new AssistantMessageEventStream<"zai">();
	...
	(async () => {
		...
		try {
			...
			const zaiStream = await client.chat.completions.create(
				{ ...params, stream: true },
			...
			for await (const chunk of zaiStream) {
				const choice = chunk.choices[0];
				if (!choice) continue;

				const delta = choice.delta as typeof choice.delta & ZaiChunkDelta;
				...
				if (chunk.usage) {
					const usage = chunk.usage as typeof chunk.usage & ZaiUsage;
			...
			if (accumulatedReasoningContent) {
				...
				if (firstChoice) {
					(firstChoice.message as typeof firstChoice.message & { reasoning_content?: string }).reasoning_content =
