...
export const streamDeepSeek: StreamFunction<"deepseek"> = (
	...
	const stream = new AssistantMessageEventStream<"deepseek">();
	...
	(async () => {
		...
		try {
			...
			for await (const chunk of deepseekStream) {
				...
				const delta = choice.delta as typeof choice.delta & DeepSeekChunkDelta;
				...
				if (chunk.usage) {
					const usage = chunk.usage as typeof chunk.usage & DeepSeekUsage;
