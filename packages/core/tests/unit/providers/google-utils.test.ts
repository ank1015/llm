...
describe("Google Utils", () => {
		...
		describe("user messages", () => {
			...
			it("should convert user file to inlineData", () => {
				const userMessage: UserMessage = {
					role: "user",
					id: "msg-1",
					content: [{ type: "file", data: "pdfdata", mimeType: "application/pdf", filename: "doc.pdf" }],
				};
				const context: Context = { messages: [userMessage] };
	...
	describe("convertTools", () => {
		it("should wrap tools in functionDeclarations", () => {
			...
			expect((result[0] as any).functionDeclarations[0].name).toBe("search");
			expect((result[0] as any).functionDeclarations[0].description).toBe("Search the web");
			expect((result[0] as any).functionDeclarations[0].parameters).toBeDefined();
			expect((result[0] as any).functionDeclarations[0].parameters.type).toBe("object");
		});
		...
		it("should transform schemas for Google compatibility", () => {
			...
			const result = convertTools([tool]);
			const params: any = (result[0] as any).functionDeclarations[0].parameters;
	...
	describe("getAssistantStopReason", () => {
		...
		it("should use first candidate when multiple exist", () => {
			const response: GenerateContentResponse = {
				candidates: [{ finishReason: FinishReason.MAX_TOKENS }, { finishReason: FinishReason.STOP }],
			} as GenerateContentResponse;
