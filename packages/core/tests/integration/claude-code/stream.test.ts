...
describe('Claude Code Stream Integration', () => {
  ...
  function streamClaudeCode(
    ...
    return coreStreamClaudeCode(model, context, toClaudeCodeOptions(options), id);
  }
  ...
  describe('basic streaming', () => {
    it('should emit start event first', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 100 }, 'test-msg-1');

      const events: BaseAssistantEvent<'claude-code'>[] = [];
    ...
    it('should emit text_start before text deltas', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 100 }, 'test-msg-2');
    ...
    it('should emit text_delta with incremental text', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 100 }, 'test-msg-3');
    ...
    it('should emit text_end with complete content', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 100 }, 'test-msg-4');

      let textEndEvent: BaseAssistantEvent<'claude-code'> | undefined;
      for await (const event of stream) {
    ...
    it('should emit done event at the end', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 100 }, 'test-msg-5');
  ...
  describe('result() promise', () => {
    it('should return complete BaseAssistantMessage', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 100 }, 'test-msg-6');
      ...
      expect(result.api).toBe('claude-code');
    ...
    it('should calculate usage in final result', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 100 }, 'test-msg-7');
    ...
    it('should be awaitable without consuming stream', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 100 }, 'test-msg-8');
    ...
    it('should populate message.content with native Claude Code content blocks', async () => {
      const context: Context = {
        messages: [
          {
            role: 'user',
      ...
      // Verify native Claude Code message content is populated (bug fix verification)
      expect(result.message).toBeDefined();
      expect(result.message.content).toBeDefined();
      expect(Array.isArray(result.message.content)).toBe(true);
      expect(result.message.content.length).toBeGreaterThan(0);

      // Verify the content has the expected Claude Code structure
    ...
    it('should capture message metadata (id, model) from stream', async () => {
      ...
      expect(result.message.id).toMatch(/^msg_/); // Claude Code message IDs start with msg_
  ...
  describe('tool call streaming', () => {
    it('should emit toolcall_start/delta/end for function calls', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 500 }, 'test-msg-9');
    ...
    it('should parse partial JSON during streaming', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 500 }, 'test-msg-10');
    ...
    it('should return toolUse stop reason when tool is called', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 500 }, 'test-msg-11');
  ...
  describe('abort handling', () => {
    it('should handle abort signal', async () => {
      ...
      const stream = streamClaudeCode(
        model,
        context,
        { apiKey, max_tokens: 2000, signal: controller.signal },
        'test-msg-12'
      );
        ...
        messages: [
          {
            ...
            id: 'test-1',
            content: [
              { type: 'text', content: 'Write a long essay about the history of computing' },
            ],
          },
        ],
      };
  ...
  describe('message updates', () => {
    it('should update message timestamp in events', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 100 }, 'test-msg-16');
    ...
    it('should include contentIndex in delta events', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 100 }, 'test-msg-17');
  ...
  describe('system prompt handling', () => {
    it('should handle system prompt in streaming', async () => {
      ...
      const stream = streamClaudeCode(model, context, { apiKey, max_tokens: 200 }, 'test-msg-18');
  ...
  describe('usage tracking during stream', () => {
    it('should have usage data in done event', async () => {
      ...
      let doneEvent: BaseAssistantEvent<'claude-code'> | undefined;
      for await (const event of stream) {
        if (event.type === 'done') {
          doneEvent = event;
        }
    ...
    it('should preserve input tokens from message_start in final usage', async () => {
      ...
      const stream = streamClaudeCode(
        model,
        context,
        { apiKey, max_tokens: 200 },
        'test-msg-input-tokens'
      );
        ...
        messages: [
          {
            ...
            content: [
              {
                type: 'text',
                content: 'I want to check the weather in London. Can you help me with that?',
              },
            ],
          },
        ],
