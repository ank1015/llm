...
describe("Kimi Complete Integration", () => {
	...
	beforeAll(() => {
		...
		// Use the kimi-k2.5 model for testing
	...
	describe("basic completion", () => {
		it("should return valid BaseAssistantMessage", async () => {
			...
			const result = await completeKimi(model, context, { apiKey }, "test-msg-1");
		...
		it("should include native ChatCompletion in message field", async () => {
			...
			const result = await completeKimi(model, context, { apiKey }, "test-msg-2");
		...
		it("should calculate duration correctly", async () => {
			...
			const result = await completeKimi(model, context, { apiKey }, "test-msg-3");
		...
		it("should return text content", async () => {
			...
			const result = await completeKimi(model, context, { apiKey }, "test-msg-4");
		...
		it("should handle system prompt", async () => {
			...
			const result = await completeKimi(model, context, { apiKey }, "test-msg-5");
	...
	describe("usage tracking", () => {
		it("should track token usage", async () => {
			...
			const result = await completeKimi(model, context, { apiKey }, "test-msg-6");

			expect(result.usage).toBeDefined();
			expect(result.usage.input || result.usage.cacheRead).toBeGreaterThan(0);
		...
		it("should calculate cost", async () => {
			...
			const result = await completeKimi(model, context, { apiKey }, "test-msg-7");
	...
	describe("tool calling", () => {
		it("should execute and return tool calls", async () => {
			...
			const result = await completeKimi(model, context, { apiKey }, "test-msg-8");
		...
		it("should validate tool call arguments", async () => {
			...
			const result = await completeKimi(model, context, { apiKey }, "test-msg-9");
	...
	describe("thinking/reasoning", () => {
		it("should support thinking with enabled config", async () => {
			const context: Context = {
				messages: [
					{
						role: "user",
		...
		it("should work with thinking disabled", async () => {
			const context: Context = {
				messages: [
					{
						role: "user",
	...
	describe("error handling", () => {
		it("should handle API errors gracefully", async () => {
			...
			const result = await completeKimi(model, context, { apiKey: "invalid-key-12345" }, "test-msg-10");
		...
		it("should handle abort signal", async () => {
			...
			const result = await completeKimi(model, context, { apiKey, signal: controller.signal }, "test-msg-11");
	...
	describe("multi-turn conversations", () => {
		it("should handle conversation context", async () => {
			...
			const result = await completeKimi(model, context, { apiKey }, "test-msg-12");
		...
		it("should handle preserved thinking in conversation history", async () => {
			...
			const result = await completeKimi(model, context, { apiKey }, "test-msg-13");
	...
	describe("cross-provider handoff", () => {
		it("should handle conversation with OpenAI assistant message in history", async () => {
			...
			const result = await completeKimi(model, context, { apiKey }, "test-handoff-1");
		...
		it("should handle cross-provider handoff with thinking content", async () => {
			...
			const result = await completeKimi(model, context, { apiKey }, "test-handoff-think-1");
