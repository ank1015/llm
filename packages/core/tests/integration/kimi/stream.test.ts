...
describe("Kimi Stream Integration", () => {
	...
	describe("basic streaming", () => {
		it("should emit start event first", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-1");

			const events: BaseAssistantEvent<"kimi">[] = [];
		...
		it("should emit text_start before text deltas", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-2");
		...
		it("should emit text_delta with incremental text", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-3");
		...
		it("should emit text_end with complete content", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-4");

			let textEndEvent: BaseAssistantEvent<"kimi"> | undefined;
			for await (const event of stream) {
		...
		it("should emit done event at the end", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-5");
	...
	describe("result() promise", () => {
		it("should return complete BaseAssistantMessage", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-6");
		...
		it("should calculate usage in final result", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-7");
		...
		it("should be awaitable without consuming stream", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-8");
	...
	describe("tool call streaming", () => {
		it("should emit toolcall_start/delta/end for function calls", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-9");
		...
		it("should parse partial JSON during streaming", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-10");
		...
		it("should validate tool arguments on completion", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-11");
	...
	describe("thinking/reasoning streaming", () => {
		it("should emit thinking_start/delta/end with thinking enabled", async () => {
			...
			const stream = streamKimi(model, context, { apiKey, thinking: { type: "enabled" } }, "test-msg-12");
			...
			// Kimi K2.5 with thinking enabled should have thinking events
		...
		it("should accumulate reasoning_content in final message", async () => {
			...
			const stream = streamKimi(model, context, { apiKey, thinking: { type: "enabled" } }, "test-msg-13");
		...
		it("should preserve reasoning_content in native message", async () => {
			const context: Context = {
				messages: [
					{
						role: "user",
			...
			const stream = streamKimi(model, context, { apiKey, thinking: { type: "enabled" } }, "test-msg-14");
	...
	describe("abort handling", () => {
		it("should handle abort signal", async () => {
			...
			const stream = streamKimi(model, context, { apiKey, signal: controller.signal }, "test-msg-15");
		...
		it("should emit error event on abort", async () => {
			...
			const controller = new AbortController();
			const stream = streamKimi(model, context, { apiKey, signal: controller.signal }, "test-msg-16");
	...
	describe("error handling", () => {
		it("should emit error event on API error", async () => {
			...
			const stream = streamKimi(model, context, { apiKey: "invalid-key-12345" }, "test-msg-17");
		...
		it("should return error result on API error", async () => {
			...
			const stream = streamKimi(model, context, { apiKey: "invalid-key-12345" }, "test-msg-18");
	...
	describe("message updates", () => {
		it("should update message timestamp in events", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-19");
		...
		it("should include contentIndex in delta events", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-20");
	...
	describe("system prompt handling", () => {
		it("should handle system prompt in streaming", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-21");
	...
	describe("native message format", () => {
		it("should include ChatCompletion in final result message", async () => {
			...
			const stream = streamKimi(model, context, { apiKey }, "test-msg-22");
		...
		it("should include reasoning_content in message when thinking enabled", async () => {
			...
			const stream = streamKimi(model, context, { apiKey, thinking: { type: "enabled" } }, "test-msg-23");
