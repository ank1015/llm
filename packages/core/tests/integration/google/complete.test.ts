...
describe("Google Complete Integration", () => {
						...
						content: [
							{
								type: "text",
								content:
									"Use the get_weather tool to check the weather in San Francisco. You must use the tool.",
							},
						],
					},
				],
	...
	describe("error handling", () => {
		it("should handle API errors gracefully", async () => {
			...
			const result = await completeGoogle(model, context, { apiKey: "invalid-key-12345" }, "test-msg-10");
		...
		it("should handle abort signal", async () => {
			...
			const result = await completeGoogle(model, context, { apiKey, signal: controller.signal }, "test-msg-11");
