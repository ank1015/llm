...
describe('Google Conversation Integration', () => {
  it('should use calculator tool correctly', async () => {
    ...
    // "What is 15 * 4?" -> expect tool call -> expect answer 60
    const messages = await conversation.prompt('What is 15 * 4? Use the calculator tool.');
    ...
    expect(textContent.content).toContain('60');
  ...
  it('should use get_current_time tool', async () => {
    ...
    const messages = await conversation.prompt('What time is it now?');
