...
import { TextContent, Content, BaseAssistantMessage, Api, BaseAssistantEvent, AgentEvent } from '@ank1015/llm-types';
...
describe('Kimi Conversation Integration', () => {
    ...
    it('should handle preserved thinking across turns', async () => {
        ...
        const responseContent = (secondLastMessage as BaseAssistantMessage<Api>).content.find(c => c.type === 'response');
        const textContent = responseContent?.content.find((c) => c.type === 'text') as TextContent;
        expect(textContent).toBeDefined();
