...
describe('Kimi Conversation Integration', () => {
  it('should use calculator tool correctly', async () => {
    ...
    const responseContent = (lastMessage as BaseAssistantMessage<Api>).content.find(
      (c) => c.type === 'response'
    );
  ...
  it('should emit all agent events correctly during tool use', async () => {
    ...
    const userMessageStarts = events.filter(
      (e) => e.type === 'message_start' && e.messageType === 'user'
    );
    const userMessageEnds = events.filter(
      (e) => e.type === 'message_end' && e.messageType === 'user'
    );
    ...
    const assistantMessageStarts = events.filter(
      (e) => e.type === 'message_start' && e.messageType === 'assistant'
    );
    const assistantMessageEnds = events.filter(
      (e) => e.type === 'message_end' && e.messageType === 'assistant'
    );
    ...
    const assistantMessageUpdates = events.filter(
      (e) => e.type === 'message_update' && e.messageType === 'assistant'
    );
    ...
    const toolResultStarts = events.filter(
      (e) => e.type === 'message_start' && e.messageType === 'toolResult'
    );
    const toolResultEnds = events.filter(
      (e) => e.type === 'message_end' && e.messageType === 'toolResult'
    );
  ...
  it('should handle reasoning/thinking mode', async () => {
    ...
    // Kimi K2.5 with thinking enabled should have thinking content
    const thinkingContent = (lastMessage as BaseAssistantMessage<Api>).content.find(
      (c) => c.type === 'thinking'
    );
  ...
  it('should emit thinking events during reasoning', async () => {
    ...
    // Kimi K2.5 with thinking enabled should produce thinking for this query
  ...
  it('should handle system prompt', async () => {
    ...
    const hasThePirateSpirit = pirateWords.some((word) =>
      textContent.content.toLowerCase().includes(word)
    );
    expect(hasThePirateSpirit).toBe(true);
  }, 60000);

  it('should handle preserved thinking across turns', async () => {
    const conversation = new Conversation();
    const model = MODELS.kimi['kimi-k2.5'];
    ...
    const firstMessages = await conversation.prompt('What is 10 + 5? Think step by step.');

    const firstLastMessage = firstMessages[firstMessages.length - 1] as BaseAssistantMessage<Api>;
    expect(firstLastMessage.role).toBe('assistant');

    // Check if reasoning_content is preserved in the native message
    const nativeMessage = (firstLastMessage.message as any).choices?.[0]?.message as any;
    if (nativeMessage?.reasoning_content) {
      expect(typeof nativeMessage.reasoning_content).toBe('string');
    }

    // Second turn should still work with preserved thinking context
    const secondMessages = await conversation.prompt('Now add 3 to that result.');

    const secondLastMessage = secondMessages[secondMessages.length - 1];
    expect(secondLastMessage.role).toBe('assistant');

    const responseContent = (secondLastMessage as BaseAssistantMessage<Api>).content.find(
      (c) => c.type === 'response'
    );
    const textContent = responseContent?.content.find((c) => c.type === 'text') as TextContent;
    expect(textContent).toBeDefined();
    // Should reference the previous result (15) + 3 = 18
    expect(textContent.content).toContain('18');
  }, 120000);

  it('should handle disabled thinking mode', async () => {
    const conversation = new Conversation();
    const model = MODELS.kimi['kimi-k2.5'];
    ...
    const messages = await conversation.prompt('What is 5 + 3?');

    const lastMessage = messages[messages.length - 1];
    expect(lastMessage.role).toBe('assistant');

    // With thinking disabled, there should be no thinking content
    const thinkingContent = (lastMessage as BaseAssistantMessage<Api>).content.find(
      (c) => c.type === 'thinking'
    );
  ...
  it('should work with kimi-k2-turbo-preview model', async () => {
    ...
    const messages = await conversation.prompt('What is the capital of France?');
    ...
    const textContent = responseContent?.content.find((c) => c.type === 'text') as TextContent;
    expect(textContent).toBeDefined();
    expect(textContent.content.toLowerCase()).toContain('paris');
  }, 60000);
});
