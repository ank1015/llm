...
app.post("/complete", async (c) => {
	try {
		...
		if (error instanceof LLMError) {
			return c.json(error.toResponse(), error.statusCode as 400 | 401 | 404 | 413 | 429 | 500 | 502);
		}
		...
		return streamSSE(c, async (sseStream) => {
			try {
				...
				if (isContextOverflow(finalMessage, model.contextWindow)) {
					await sseStream.writeSSE({
						event: "error",
						data: JSON.stringify(new ContextOverflowError(request.api, finalMessage.usage.input, model.contextWindow).toResponse()),
					});
				}
