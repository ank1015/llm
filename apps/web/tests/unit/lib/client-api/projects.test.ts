...
describe('projects client-api', () => {
  ...
  it('throws when an API endpoint returns a non-JSON success response', async () => {
    delete process.env.NEXT_PUBLIC_LLM_SERVER_BASE_URL;

    const fetchMock = vi.fn().mockResolvedValue(
      new Response('<!DOCTYPE html><title>Not API</title>', {
        headers: {
    ...
    const { listProjects } = await import('@/lib/client-api/projects');

    await expect(listProjects()).rejects.toThrow(
      'Expected JSON response from http://localhost:8001/api/projects'
    );
  });
