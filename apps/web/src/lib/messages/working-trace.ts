...
function extractTerminalAssistantResponseText(
  ...
  const responseText = message.content
    ...
    .map((block) =>
      block.response
        .filter((part): part is Extract<(typeof block.response)[number], { type: "text" }> => {
...
function extractToolImages(content: Content) {
  return content.filter((part): part is Extract<Content[number], { type: "image" }> => {
    return part.type === "image";
  });
}

function extractToolFiles(content: Content) {
  return content.filter((part): part is Extract<Content[number], { type: "file" }> => {
...
export function buildWorkingTraceModel(input: BuildWorkingTraceInput): WorkingTraceModel {
  ...
  const persistedFinalAssistant =
    input.assistantNode?.message.role === "assistant"
      ...
      : null;
  const liveFinalAssistant = persistedFinalAssistant ? null : getLastLiveFinalAssistant(liveEndedAssistants);
...
export function getWorkingTraceImages(content: Content) {
  return extractToolImages(content);
}

export function getWorkingTraceFiles(content: Content) {
