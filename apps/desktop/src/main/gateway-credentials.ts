...
const credentialsPath = join(homedir(), '.llm', 'gateway.json');
...
export const signOutGateway = async (): Promise<GatewaySession> => {
...
const requestGatewayLogin = async (
  ...
  const response = await fetch(`${defaultGatewayBaseUrl}/v1/auth/login`, {
    method: 'POST',
    headers: {
...
const isGatewayCredentials = (value: unknown): value is GatewayCredentials => {
  ...
  const record = value;
  const hasTokenPair = isGatewayLoginResponse(record);
  const hasMetadata =
    typeof record['gatewayBaseUrl'] === 'string' && typeof record['savedAt'] === 'string';
